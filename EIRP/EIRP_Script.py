...
#sweep time, rbw, test antenna gain, bench offset values
def eirp_script(center_freq,
    ...
    adj_bw = float(adj_bw)
    alt_bw = float(alt_bw)
    adj_space = float(adj_space)
    alt_space = float(alt_space)
    ...
    #create two channels next to the TX channel,
    #adjacent and alternate (mirrored on each side)
    success = write_command( Analyzer, "POW:ACH:ACP 2" )
    #name the channels
    success = write_command( Analyzer, "POW:ACH:NAME:ACH 'Adj'" )
    success = write_command( Analyzer, "POW:ACH:NAME:ALT1 'ALT1'" )

# python script created by FSW: 02:07:2019 16:23:45
...
success = write_command( Analyzer, (":INST:CRE:NEW SANALYZER '%s' ") %("SC CP ACLR") )
success = write_command( Analyzer, ":INIT:CONT OFF" )
...
success = write_command( Analyzer, ":SENS:POW:ACH:BWID:CHAN1 99840000" )    #99.84MHz Carrier BW
success = write_command( Analyzer, ":SENS:POW:ACH:BWID:ACH 99840000" )      #99.84MHz ACP BW
success = write_command( Analyzer, ":SENS:POW:ACH:SPAC:CHAN1 100000000" )   #100MHz Spacing btwn Carrier and ACP frequency
...
#RBW defined
success = write_command( Analyzer, ":SENS:BAND:RES 1000000" )   #1MHz RBW.  Value may change based on 3gPP standards
...
success = write_command( Analyzer, ":SENS:FREQ:CENT 37050000000" )  #Freqency set to center frequency
